...
Start with Slice 1 because it makes the existing system readable without changing governance semantics:
...
Then wire the generated `reports/<feature>/harness.md` into:

```text
.hermes/workflows/<feature>/09-harness-report.md
Feishu ledger latest evidence link
final-verification evidence citation
```
