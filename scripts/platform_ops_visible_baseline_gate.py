...
def main() -> int:
    ...
    checks = {
        ...
        "audit_diagnostics_frontend_entrypoints": {"has_log_env": "VITE_LOG_EXPLORER_URL" in env_text, "has_trace_env": "VITE_TRACE_EXPLORER_URL" in env_text, "has_trace_backend_gate": "VITE_TRACE_BACKEND_ENABLED" in env_text and "traceBackendEnabled" in audit_page_text, "has_system_log_search": "system-log-search-panel" in audit_page_text and "X-Request-ID" in audit_page_text, "has_open_logs_action": "Open logs" in audit_page_text, "has_open_trace_action_or_disabled_state": "Open trace" in audit_page_text and "Trace backend not enabled" in audit_page_text, "frontend_avoids_query_language_coupling": "LogQL" not in audit_page_text},
    ...
    expectations = {
        ...
        "audit_diagnostics_external_log_trace_entry_configurable": bool("VITE_LOG_EXPLORER_URL" in env_text and "VITE_TRACE_EXPLORER_URL" in env_text and "VITE_TRACE_BACKEND_ENABLED" in env_text and "system-log-search-panel" in audit_page_text and "X-Request-ID" in audit_page_text and "Open logs" in audit_page_text and "Trace backend not enabled" in audit_page_text and "LogQL" not in audit_page_text),
